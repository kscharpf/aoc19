...
def eval_instruction(prog_data, prog_idx, inputQ, outputQ, relative_base):
    ...
    if opcode == 1:
        ...
        print(f"ADD {outp_relbase + outp[0]} set to {inp[0] + inp[1]}")
    elif opcode == 2:
        assert(num_inp == 2)
        assert(num_outp == 1)
        prog_data[outp_relbase + outp[0]] = inp[0] * inp[1]
        print(f"MUL {outp_relbase + outp[0]} set to {inp[0] + inp[1]}")
    elif opcode == 3:
        if len(inputQ) == 0:
            # ask the user
            print("INPUT: ", end="")
        ...
        print(f"READ {outp_relbase + outp[0]} set to {v}")
    elif opcode == 5:
        if inp[0] != 0:
            new_prog_idx = inp[1]
            print(f"JUMP_NZ new index {new_prog_idx}")
        else:
            print(f"JUMP_NZ ignored")
    elif opcode == 6:
        if inp[0] == 0:
            new_prog_idx = inp[1]
            print(f"JUMP_EQ new index {new_prog_idx}")
        else:
            print(f"JUMP_EQ ignored")
    elif opcode == 7:
        prog_data[outp_relbase + outp[0]] = 1 if inp[0] < inp[1] else 0
        print(f"SET_LT idx {outp_relbase + outp[0]}")
    elif opcode == 8:
        prog_data[outp_relbase + outp[0]] = 1 if inp[0] == inp[1] else 0
        print(f"SET_EQ idx {outp_relbase + outp[0]}")
    elif opcode == 9:
        relative_base += inp[0]
...
def eval_program(prog_data, prog_idx, inputQ, relative_base):
    ...
    while prog_idx < len(prog_data):
        print(f"Idx {prog_idx} {prog_data[prog_idx:prog_idx+4]}")
